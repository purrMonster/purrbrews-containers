...
if [[ ! -f "${DIR}/.env.local" ]]; then
  ...
  # never pick it up. Bug fixed 2026-09-16 — same key-sync step sieve's
  # and percolator's setup-secrets.sh already had; cellar/mochaPot/
  # grinder/roastery were missing it. (setup-secrets.ps1 needs the same
  # fix — keep them in sync by hand, see that file's own header comment.)
  added=0
