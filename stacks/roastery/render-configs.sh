...
# into its real counterpart, substituting from .env.local and each app's
# own secrets.env.local. Identical mechanics to every other node's
# render-configs.sh. No app on roastery has a config template yet -- kept
# here for consistency, and for whenever Ollama or Jellyfin lands with one.
#
...
[[ "$FOUND" -eq 1 ]] || echo "  no *.template files to render"
