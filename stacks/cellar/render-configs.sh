...
# into its real counterpart (e.g. config/foo.yml.template ->
# config/foo.yml), substituting ${DOMAIN}/${CELLAR_LAN_IP}/${TZ}/etc from
# .env.local and each app's own secrets.env.local. Identical to every other
# node's render-configs.sh — copied verbatim, nothing cellar-specific in
# the logic itself.
#
